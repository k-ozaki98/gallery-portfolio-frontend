...
export default function AdminDashboard() {
  ...
  const fetchPendingPortfolios = async () => {
    try {
      const response = await fetch(`${API_URL}/admin/portfolios/pending`, {
        headers: {
  ...
  const handleApprove = async (id) => {
    try {
      const response = await fetch(`${API_URL}/admin/portfolios/${id}/approve`, {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`
        }
  ...
  const handleReject = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL}/admin/portfolios/${id}/reject`, {
        method: 'POST',
        headers: {
